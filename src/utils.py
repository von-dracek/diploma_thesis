...
from src.tree_generation import (
    create_empty_tree,
    fill_empty_tree_with_scenario_data_moment_matching,
)
...
def get_necessary_data(
    ...
    data = datagetter.randomly_sample_data(
        random_generator, train_or_test, defined_tickers, train_or_test_time
    )
    return data


def get_cvar_value(
    gams_workspace, branching: List[int], data, alpha: float = 0.95
) -> float:
    ...
    root = fill_empty_tree_with_scenario_data_moment_matching(
        TARMOM, R, tree_root, branching, gams_workspace
    )
