    ...
    if perform_experiment:
        for asset_set_index, asset_set in enumerate(asset_sets):
            for env_name, env_function in env_function_dict.items():
                env = env_function(defined_tickers=asset_set, defined_alpha=0.9, train_or_test="train", train_or_test_time="train")()
        ...
        for i, plot_string in enumerate(plots_to_produce):
            with open(f'{plot_string}.pickle', 'rb') as handle:
                data = pickle.load(handle)
                data = pd.DataFrame(data)
                fig.add_trace(go.Scatter(x=list(data[0]), y=list(data[1]/min(data[1])), marker=dict(color='grey',line=dict(width=2, color='DarkSlateGrey')), mode="markers"), row=i+1, col=1)
        ...
        for i, plot_string in enumerate(plots_to_produce):
            with open(f'{plot_string}.pickle', 'rb') as handle:
                data = pickle.load(handle)
                data = pd.DataFrame(data)
                fig.add_trace(go.Scatter(mode="markers", marker=dict(color='grey',line=dict(width=2, color='DarkSlateGrey')), x=list(data[2].apply(len)), y=list(data[1]/min(data[1]))), row=i+1, col=1)
        fig.update_layout(template="simple_white")
        for i in range(1,4):
            for j in range(1,3):
                if j==1:
        ...
        for i, plot_string in enumerate(plots_to_produce):
            with open(f'{plot_string}.pickle', 'rb') as handle:
                data = pickle.load(handle)
                data = pd.DataFrame(data)
                fig.add_trace(go.Box(fillcolor="white", marker=dict(color='grey',line=dict(width=2, color='DarkSlateGrey')), boxpoints="all", x=list(data[2].apply(len)), y=list(data[1]/min(data[1]))), row=i+1, col=1)
        fig.update_layout(template="simple_white")
        for i in range(1,4):
            for j in range(1,3):
                if j == 1:
        ...
        for i, plot_string in enumerate(plots_to_produce):
            with open(f'{plot_string}.pickle', 'rb') as handle:
                data = pickle.load(handle)
                data = pd.DataFrame(data)
                fig.add_trace(go.Scatter(x=list(data[0]), y=list(data[1]), marker=dict(color='grey',line=dict(width=2, color='DarkSlateGrey')), mode="markers"), row=i+1, col=1)
        fig.update_layout(template="simple_white")
        for i in range(1,4):
            for j in range(1,3):
                if j == 1:
        ...
        for i, plot_string in enumerate(plots_to_produce):
            with open(f'{plot_string}.pickle', 'rb') as handle:
                data = pickle.load(handle)
                data = pd.DataFrame(data)
                fig.add_trace(go.Scatter(mode="markers", marker=dict(color='grey',line=dict(width=2, color='DarkSlateGrey')), x=list(data[2].apply(len)), y=list(data[1])), row=i+1, col=1)
        fig.update_layout(template="simple_white")
        for i in range(1,4):
            for j in range(1,3):
                if j == 1:
