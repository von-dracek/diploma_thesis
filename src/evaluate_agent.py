...
from src.reinforcement_environment import (
    TreeBuildingEnv,
    _reward_func_pretraining,
    _reward_func_v2,
    penalty_func_linear,
    penalty_func_quadratic,
)

dir_quaadratic_penalty = (
    "./tensorboard_logging/gym_final_with_quadratic_penalty/agent_evaluation/"
)
# dir_no_penalty = "./tensorboard_logging/gym_/agent_evaluation/"
dir_linear_penalty = "./tensorboard_logging/gym_linear_01/agent_evaluation/"
    ...
    for train_or_test_tickers in ["train", "test"]:
        for train_or_test_time in ["train", "test"]:
            ...
            if start_evaluation:
                ...
                env = make_treebuilding_env(
                    train_or_test=train_or_test_tickers,
                    train_or_test_time=train_or_test_time,
                    penalty_func=penalty_func_quadratic
                    if dir == dir_quaadratic_penalty
                    else (penalty_func_linear if dir == dir_linear_penalty else None),
                )
                ...
                if run_ppo:
                    ...
                    for episode in range(n_eval_episodes):
                        ...
                        if hasattr(
                            env.venv.venv.envs[0].unwrapped.unwrapped, "penalty_func"
                        ):
                            penalty_func = env.venv.venv.envs[
                                0
                            ].unwrapped.unwrapped.penalty_func
                            penalty = penalty_func(info[0]["num_scen"])
                            reward_without_penalty = rew + penalty
                        else:
                            reward_without_penalty = rew
                        final_ppo.append(
                            {
                ...
                if run_random:
                    ...
                    env = make_treebuilding_env(
                        ...
                        if dir == dir_quaadratic_penalty
                            ...
                            penalty_func_linear if dir == dir_linear_penalty else None
                        ),
                    )
                    venv = DummyVecEnv(env_fns=[env] * 1)
                    venv = VecMonitor(venv, log_dir, info_keywords=("num_scen",))
                    env = VecNormalize.load(
                        os.path.join(log_dir, "latest_env"), venv=venv
                    )
                    env.seed(1337)
                    ...
                    for episode in range(n_eval_episodes):
                        print(
                            f"Running episode {episode} out of {n_eval_episodes} Random"
                        )
                        done = False
                        obs = env.reset()
                        while not done:
                            ...
                            action = (
                                np.random.choice(valid_actions) - 3
                ...
                if run_ppo:
                    with open(
                        f"{log_dir}ppo_rewards_before_assert_{train_or_test_tickers}_{train_or_test_time}_{current_time}.pckl",
                        "wb",
                    ) as f:
                ...
                if run_random:
                    with open(
                        f"{log_dir}random_rewards_before_assert_{train_or_test_tickers}_{train_or_test_time}_{current_time}.pckl",
                        "wb",
                    ) as f:
                        pickle.dump(final_random, f)

                if run_ppo and run_random:
                    assert len(predictors_ppo) == len(predictors_random)
                    assert all(
                        [
                            np.allclose(a, b, rtol=1e-2, atol=1e-2)
                            for a, b in zip(predictors_ppo, predictors_random)
                        ]
                    ), "predictors differ in evaluation between random agent and ppo"

                if run_ppo:
                    with open(
                        f"{log_dir}ppo_rewards_evaluated_{train_or_test_tickers}_{train_or_test_time}_{current_time}.pckl",
                        "wb",
                    ) as f:
                        pickle.dump(final_ppo, f)
                ...
                if run_random:
                    with open(
                        f"{log_dir}random_rewards_evaluated_{train_or_test_tickers}_{train_or_test_time}_{current_time}.pckl",
                        "wb",
                    ) as f:
                        pickle.dump(final_random, f)
                if run_ppo:
                    with open(
                        f"{log_dir}ppo_rewards_evaluated_{train_or_test_tickers}_{train_or_test_time}_latest.pckl",
                        "wb",
                    ) as f:
                        pickle.dump(final_ppo, f)
                if run_random:
                    with open(
                        f"{log_dir}random_rewards_evaluated_{train_or_test_tickers}_{train_or_test_time}_latest.pckl",
                        "wb",
                    ) as f:
                        pickle.dump(final_random, f)
                if run_random and run_ppo:
                ...
                def parse_dataset(dataset):
                    return {
                        "predictors": [x["obs"]["predictors"] for x in dataset],
                        "rewards": [x["rew"][0] for x in dataset],
                        "number_scenarios": [x["info"][0]["num_scen"] for x in dataset],
                        "terminal_observations": [
                            x["info"][0]["terminal_observation"] for x in dataset
                        ],
                        "reward_without_penalty": [
                            x["reward_without_penalty"][0] for x in dataset
                        ],
                    }

                def terminal_observations_to_branching(terminal_observations):
                    states_reshaped = [
                        x["state"].reshape(8, 8) for x in terminal_observations
                    ]
                    branchings = [
                        np.argmax(state[1:, :], axis=1) for state in states_reshaped
                    ]
                ...
                ppo_parsed["branchings"] = terminal_observations_to_branching(
                    ppo_parsed["terminal_observations"]
                )
                random_parsed["branchings"] = terminal_observations_to_branching(
                    random_parsed["terminal_observations"]
                )

                # plot_titles=["Reward barplot train train", "Reward scatterplot train train"]
                log_dir = (
                    log_dir + f"graphs_{train_or_test_time}_{train_or_test_tickers}/"
                )
                os.makedirs(log_dir, exist_ok=True)

                data_plot1 = pd.DataFrame(
                    {
                        "reinforcement_agent": ppo_parsed["rewards"],
                        ...
                        "reinforcement_agent_without_penalty": ppo_parsed[
                            "reward_without_penalty"
                        ],
                        "random_agent_without_penalty": random_parsed[
                            "reward_without_penalty"
                        ],
                    }
                )
                ...
                assert all(
                    [
                        np.allclose(a, b, rtol=1e-2, atol=1e-2)
                        for a, b in zip(
                            ppo_parsed["predictors"], random_parsed["predictors"]
                        )
                    ]
                ), "predictors differ in evaluation between random agent and ppo"

                results_str = f"ppo_mean: {data_plot1['reinforcement_agent'].mean()} \nrandom_agent_mean: {data_plot1['random_agent'].mean()} \n"
                results_str_2 = f"ppo_mean_no_penalty: {data_plot1['reinforcement_agent_without_penalty'].mean()} \nrandom_agent_mean_no_penalty: {data_plot1['random_agent_without_penalty'].mean()} "

                with open(f"{log_dir}_mean_results.txt", "w") as text_file:
                    text_file.write(results_str)
                    text_file.write(results_str_2)
                ...
                fig.add_trace(
                    go.Box(
                        fillcolor="white",
                        marker=dict(
                            color=reinforcement_agent_color,
                            line=dict(width=1, color=None),
                        ),
                        boxpoints="all",
                ...
                fig.add_trace(
                    go.Box(
                        fillcolor="white",
                        marker=dict(
                            color=random_agent_color, line=dict(width=1, color=None)
                        ),
                        boxpoints="all",
                ...
                fig.update_layout(
                    legend=dict(
                        orientation="h", yanchor="bottom", y=1.035, xanchor="right", x=1
                    )
                )
                fig.update_layout(
                    font=dict(
                ...
                fig.write_html(
                    f"{log_dir}evaluation_1_{train_or_test_time}_{train_or_test_tickers}.html"
                )
                fig.write_image(
                    f"{log_dir}evaluation_1_{train_or_test_time}_{train_or_test_tickers}.pdf"
                )
                time.sleep(3)
                fig.write_image(
                    f"{log_dir}evaluation_1_{train_or_test_time}_{train_or_test_tickers}.pdf"
                ...
                fig.add_trace(
                    go.Scatter(
                        x=list(data_plot1["reinforcement_agent_scenarios"]),
                        y=list(data_plot1["reinforcement_agent"]),
                        marker=dict(
                            color=reinforcement_agent_color,
                ...
                fig.add_trace(
                    go.Scatter(
                        x=list(data_plot1["random_agent_scenarios"]),
                        y=list(data_plot1["random_agent"]),
                        marker=dict(
                            color=random_agent_color, line=dict(width=2, color=None)
                        ),
                        mode="markers",
                        name="Random agent",
                    ),
                ...
                fig.write_html(
                    f"{log_dir}evaluation_2_{train_or_test_time}_{train_or_test_tickers}.html"
                )
                time.sleep(1)
                fig.write_image(
                    f"{log_dir}evaluation_2_{train_or_test_time}_{train_or_test_tickers}.pdf"
                )
                fig.write_image(
                    f"{log_dir}evaluation_2_{train_or_test_time}_{train_or_test_tickers}.pdf"
                )
                ...
                fig.add_trace(
                    go.Histogram(
                        ...
                        xbins=dict(  # bins used for histogram
                            start=100, end=1200, size=100
                        ),
                        name="Reinforcement agent",
                        opacity=0.5,
                        marker={"color": reinforcement_agent_color},
                    ),
                )
                fig.add_trace(
                    go.Histogram(
                        ...
                        name="Random agent",
                        opacity=0.5,
                        marker={"color": random_agent_color},
                    ),
                )
                ...
                fig.write_html(
                    f"{log_dir}evaluation_3_{train_or_test_time}_{train_or_test_tickers}.html"
                )
                time.sleep(1)
                fig.write_image(
                    f"{log_dir}evaluation_3_{train_or_test_time}_{train_or_test_tickers}.pdf"
                )
                fig.write_image(
                    f"{log_dir}evaluation_3_{train_or_test_time}_{train_or_test_tickers}.pdf"
                )
                ...
                bar_data_reinforcement_agent = (
                    data_plot1["reinforcement_agent_branchings"]
                    .apply(len)
                    .value_counts()
                )
                fig.add_trace(
                    go.Bar(
                        x=list(bar_data_reinforcement_agent.index),
                ...
                bar_data_random = (
                    data_plot1["random_agent_branchings"].apply(len).value_counts()
                )
                fig.add_trace(
                    go.Bar(
                        x=list(bar_data_random.index),
                        y=bar_data_random,
                        # marker=dict(color=random_agent_color, line=dict(width=2, color=None)),
                        # mode="markers",
                        name="Random agent",
                ...
                fig.write_html(
                    f"{log_dir}evaluation_4_{train_or_test_time}_{train_or_test_tickers}.html"
                )
                time.sleep(1)
                fig.write_image(
                    f"{log_dir}evaluation_4_{train_or_test_time}_{train_or_test_tickers}.pdf"
                )
                fig.write_image(
                    f"{log_dir}evaluation_4_{train_or_test_time}_{train_or_test_tickers}.pdf"
                )

                data_depth_boxplots = pd.DataFrame(
                    pd.concat(
                        [data_plot1["random_agent"], data_plot1["reinforcement_agent"]]
                    )
                )
                data_depth_boxplots.columns = ["reward"]
                data_depth_boxplots["color_by"] = pd.concat(
                    [
                        data_plot1["random_agent_scenarios"] * 0,
                        data_plot1["reinforcement_agent_scenarios"] * 1,
                    ]
                ).apply(lambda x: "Random agent" if x == 0 else "Reinforcement agent")
                depths = pd.concat(
                    [
                        data_plot1["random_agent_branchings"],
                        data_plot1["reinforcement_agent_branchings"],
                    ]
                ).apply(len)
                data_depth_boxplots["depth"] = depths
                branchings = pd.concat(
                    [
                        data_plot1["random_agent_branchings"],
                        data_plot1["reinforcement_agent_branchings"],
                    ]
                ...
                fig.write_html(
                    f"{log_dir}evaluation_5_{train_or_test_time}_{train_or_test_tickers}.html"
                )
                time.sleep(1)
                fig.write_image(
                    f"{log_dir}evaluation_5_{train_or_test_time}_{train_or_test_tickers}.pdf"
                )
                fig.write_image(
                    f"{log_dir}evaluation_5_{train_or_test_time}_{train_or_test_tickers}.pdf"
                )
                ...
                fig = px.box(
                    data_depth_boxplots[
                        data_depth_boxplots["color_by"] == "Random agent"
                    ],
                    x="depth",
                    y="reward",
                    color_discrete_sequence=[random_agent_color],
                )
                ...
                fig.write_html(
                    f"{log_dir}random_agent_boxplots_by_depth_{train_or_test_time}_{train_or_test_tickers}.html"
                )
                time.sleep(1)
                fig.write_image(
                    f"{log_dir}random_agent_boxplots_by_depth_{train_or_test_time}_{train_or_test_tickers}.pdf"
                )
                fig.write_image(
                    f"{log_dir}random_agent_boxplots_by_depth_{train_or_test_time}_{train_or_test_tickers}.pdf"
                )

                data_depth_boxplots_groupby = data_depth_boxplots.groupby(
                    ["color_by", "depth"]
                )
                # fig = go.Figure()
                fig = make_subplots(
                    rows=3, subplot_titles=["3 Stages", "4 Stages", "5 Stages"]
                )

                for (agent, depth), group in data_depth_boxplots_groupby:
                    levels = [x for x in range(depth)]
                    mean_in_each_level = {
                        level + 1: group["branchings"].apply(lambda x: x[level]).mean()
                        for level in levels
                    }
                    count = len(group)
                    ...
                    if count > 10:
                        fig.add_trace(
                            go.Bar(
                                x=list(mean_in_each_level.keys()),
                                y=list(mean_in_each_level.values()),
                                name=agent,
                                opacity=0.5,
                                marker={
                                    "color": reinforcement_agent_color
                                    if agent == "Reinforcement agent"
                                    else random_agent_color
                                },
                                # width=[0.5]*depth
                ...
                fig.write_html(
                    f"{log_dir}evaluation_6_{train_or_test_time}_{train_or_test_tickers}.html"
                )
                time.sleep(1)
                fig.write_image(
                    f"{log_dir}evaluation_6_{train_or_test_time}_{train_or_test_tickers}.pdf"
                )
                fig.write_image(
                    f"{log_dir}evaluation_6_{train_or_test_time}_{train_or_test_tickers}.pdf"
                )
