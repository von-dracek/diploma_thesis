...
def get_gbm_scenarios_from_TARMOM_and_R(TARMOM, R, branching):
    ...
    gbm = MultivariateGeometricBrownianMotion(dim=n_stocks, means=means, volatilities=volatilities, corr_matrix=R)
    paths = gbm.sample_paths(times=[x + 1 for x in list(range(len(branching)))], num_samples=np.prod(branching))
    paths = paths._numpy()
        ...
        if level_branching.get(i-1) is not None:
            for _k, v in level_branching.items():
                if _k ==0 or (isinstance(_k, tuple) and _k[0] == i-1):
                    ...
                    kmeansconstrained = KMeansConstrained(n_clusters=branch, size_min=data.shape[0] // branch - 1)
                    kmeansconstrained.fit(data)
                    level_branching[i, _k[i:]] = pd.DataFrame(kmeansconstrained.labels_, index=data.index).groupby(0).indices
        else:
            kmeansconstrained = KMeansConstrained(n_clusters=branch, size_min=level.shape[0]//branch - 1)
            kmeansconstrained.fit(level)
            level_branching[i] = pd.DataFrame(kmeansconstrained.labels_, index=level.index).groupby(0).indices
...
def get_gbm_scenarios_from_TARMOM_and_R_recursive(TARMOM, R, branching):
    ...
    def get_kmeans(dataframes, branching, i, subset=None):
        ...
        tmp = pd.DataFrame(kmeansconstrained.labels_,
                     index=_df.index).groupby(0).nth.groupby_object.groups
