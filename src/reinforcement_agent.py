...
# start from a random state and let the
# agent explore the state space to minimise cvar where the reward will
# be minimum cvar and also potentially a penalization for choosing a large tree

# an improvement could be achieved if the agent also
# gets characterisation of the dataset as input
...
# look at the distribution of loss function over each possible
# tree (or randomly sampled) and see if it performs better than 50th percentile

# idea for experiment
# give the agent state consisting of last N actions
# idea - If you wrote a rock-paper-scissors agent to play against a human opponent,
# you might actually formulate it as a reinforcement learning problem,
# taking the last N plays as the state, because that could
# learn to take advantage of human players' poor judgement of randomness.

# idea - structure this as a grid game problem - let the agent choose first
# the number of stages and then iteratively a branching based
# on the previous state (previously chosen branchings) and then
# receive reward after large enough number of branchings is
# reached and get a reward - we can use q learning for this
# invalid actions must be ignored - i.e. allow to choose only
# actions that lead to a sensible tree at least of length n
...
from src.configuration import (
    EPSILON_FOR_EPSILON_GREEDY_ACTION,
...
class Environment:
    ...
    def __init__(self, maximum_stages: int, branching: int):
        L = maximum_stages
        self.environment = []
        branchings = list(  # noqa: F841
            reversed(list(range(1, L)))
        )  # we require at least 3 child nodes in each stage
        # to achieve good result in moment matching
        # only generating possible branchings that are non increasing - we
        # need a good discretization of the near future while the far
        # future need not be as precise
    ...
    def __init__complex_env(self):
        L = 10
        self.environment = []
        branchings = list(reversed(list(range(3, L))))  # we require at least 3 child nodes in each
        # stage to achieve good result in moment matching
        # only generating possible branchings that are non increasing - we need a good
        # discretization of the near future while the far future need not be as precise
        for length in range(1, L):
...
class Agent:
    ...
    def _epsilon_greedy_action(self, option_probabilities: np.ndarray) -> int:
        ...
        return (
            np.argmax(option_probabilities)
            if rnd > EPSILON_FOR_EPSILON_GREEDY_ACTION
            else np.random.choice((np.arange(option_probabilities.shape[0])))
        )
