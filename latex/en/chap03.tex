...
\end{equation}
\todo{fix J(), define advantage function}
...
\begin{rem}
Notice that in the formula for $\widehat{A}(s_t,a_t;\theta,\theta_v)$ the parameters $\theta$ are not explicitly present. In reality, the rewards $r_{(\cdot)}$ depend on the policy and thus on the parameters $\theta$. \todo{is this remark correct? please check} 
\end{rem}
